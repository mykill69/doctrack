...
//     // LOGS - Super user sees all logs (like records_officer), others see filtered
//     if ($isSuperUser) {
//         $logs = Log::with('routingSlip')->get();
//     } else {
//         $logs = Log::with('routingSlip')
//             ->where(function ($query) use ($userId) {
//                 $query->where('new_user', $userId)
...
//     // COUNTS - Super user gets both counts (like records_officer), others get role-specific
//     if ($isSuperUser) {
...
//     // DOCUMENT TRACK - Super user sees all tracks (like records_officer), others see filtered
//     if ($isSuperUser) {
//         $documentTrack = Doctrack::with(['createdBy', 'doctrackFile'])
//             ->orderByDesc('created_at')
//             ->get();
...
//     // DOCTRACK COUNT - Super user counts all (like records_officer), others count filtered
//     if ($isSuperUser) {
...
// June 16, 2026 edit: optimize loading of dashboard 
...
//     // LOGS - Load with chunking for efficiency
//     $logs = collect();
...
//     // DOCUMENT TRACK - Load with chunking for efficiency
//     $documentTrack = collect();
...
public function tracking(Request $request)
{
    ...
    // 🔁 Force canonical URL like the <a href>
  ...
  $documents = Document::query()
    ...
    ->select(
        ...
        'routing_slip.user_id as routing_slip_user_id', // <--- ADD THIS
        'routing_slip.routed_users',
        'routing_slip.r_destination',
        'routing_slip.trans_remarks',
        'routing_slip.source'
...
$filteredDocuments = $documents->filter(function ($document) use ($fullName, $userId) {
    ...
    return in_array($fullName, $routedUsers) // routed_users
        || Log::where('doc_id', $document->id)
            ->whereRaw('LOWER(TRIM(new_destination)) = ?', [strtolower($fullName)])
            ->exists() // logs
        || $document->user_id == $userId // document creator
        || $document->routing_slip_user_id == $userId; // routing slip creator <-- ADDED
});
